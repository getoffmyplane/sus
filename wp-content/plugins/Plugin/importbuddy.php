 ...
 * Version: 2.0.2 - Iteration 133 - 4/29/2011
	...
	class PluginBuddyImportBuddy {
		...
		function view_step_1() {
			if ( ( $this->_defaults['password'] != '#PASSWORD#' ) && ( md5( $_POST['password'] ) != $this->_defaults['password'] ) ) {
				...
				For support see our <a href="http://anonym.to/?http://ithemes.com/codex/page/BackupBuddy" target="_blank">Knowledge Base</a> or <a href="http://anonym.to/?http://pluginbuddy.com/support/" target="_blank">Support Forum</a>.
				...
				<div style="margin-left: 20px;">
					...
					<div id="toggle-advanced" class="toggled">
						...
						<input type="checkbox" name="skip_files" <?php if ( $count == 0 ) { echo 'checked'; } ?>/> Skip zip file extraction. <?php $this->tip( 'Checking this box will result in importbuddy.php NOT extracting/unzipping your backup ZIP file for you.  You will need to manually extract it either on your local computer then upload it or using a server-based tool such as cPanel to extract it. This feature is useful if your host cannot extract the ZIP file for some reason or is timing out.' ); ?><br />
						<input type="checkbox" name="skip_database" /> Skip import & migration of database. <br />
						<!-- TODO: <input type="checkbox" name="wipe_database" /> Wipe database contents prior to import/migration. <br /> -->
		...
		function view_step_4() {
			...
			echo '<label>New Blog URL ' . $this->tip( 'This is the site address where you want the final site you are restoring / migrating to reside.', '', false ) . '</label> <input type="text" name="siteurl" value="'. $url .'" size="50" /><br /><br /><br />';
		...
		function view_step_5() {
			...
			if ( false === $this->_options['skip_database'] ) {
				$failed = !$this->restore_database( $this->_options['wipe_database'] );
			}
		 ...
		 */
		function restore_database( $wipe = false ) {
