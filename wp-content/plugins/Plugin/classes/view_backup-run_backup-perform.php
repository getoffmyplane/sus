...
if ( $backup->start_backup_process( $_GET['run_backup'], 'manual' ) !== true ) {
	echo 'Error #4344443: Backup failure';
...
<script type="text/javascript">
	...
	function backupbuddy_poll() {
		...
		if ( ( last_archive_change != 0 ) && ( ( unix_timestamp() - last_archive_change > stale_archive_time_trigger ) ) ) {
			alert( 'Warning: The backup archive file size has not increased in ' + stale_archive_time_trigger + ' seconds. The backup may have failed. If you wish to wait you will be notified if there is no progress in 5 minutes.' );
		...
		jQuery.ajax({
			...
			success: function( data ) {
							...
							if ( data[i].substring( 0, 1 ) == '!' ) { // Expected command since it begins with `!`.
								...
								if ( line[1] == 'message' ) {
									...
									if ( action_line[0] == 'archive_size' ) { // Process action sub-commands.
										...
										jQuery( '#pb_backupbuddy_slot3_led' ).removeClass( 'pb_backupbuddy_blinkz' ); // disable blinking
										keep_polling = 0; // Stop polling server for status updates.
										message_output = 'A fatal error has been encountered.  The backup has halted.';
										alert( 'A fatal error has been encountered.  The backup has halted.' );
									} else {
										details_output = 'Unknown action: ' + action_line[0] + "\n";
									}
						...
						setTimeout( 'backupbuddy_poll()' , 5000 );
					 },
			complete: function( jqXHR, status ) {
				if ( ( status != 'success' ) && ( status != 'notmodified' ) ) {
					jQuery('#pb_backupbuddy_loading').hide();
					alert( 'BackupBuddy encountered the following error receiving data from the server: ' . status );
				}
			}
		});
	}
...
<div>
	...
	<div class="pb_backupbuddy_status light L" id="pb_backupbuddy_slot1">
		...
		<div class="pb_backupbuddy_status_header light" id="pb_backupbuddy_slot1_header">Settings export</div>
	...
	<div class="pb_backupbuddy_status M" id="pb_backupbuddy_slot2">
		...
		<div class="pb_backupbuddy_status_header" id="pb_backupbuddy_slot2_header">Database export</div>
	...
	<div class="pb_backupbuddy_status M" id="pb_backupbuddy_slot3">
		...
		<div class="pb_backupbuddy_status_header" id="pb_backupbuddy_slot3_header">Files Export</div>
...
<table width="793"><tr>
	<td><span style="font-size: 1.17em; font-weight: bold;">Status</span></td>
	<td width="16"><span id="pb_backupbuddy_loading" style="display: none; margin-left: 10px;"><img src="<?php echo $this->_pluginURL; ?>/images/loading.gif" alt="Loading..." title="Loading..." width="16" height="16" style="vertical-align: -3px;" /></span></td>
	<td width="100%" align="right">Archive size: <span class="backupbuddy_archive_size">0 MB</span> <?php $this->tip( 'This is the current size of the backup archive as it is being generated. This size will grow until the backup is complete.' ); ?></td>
</tr></table>
...
<div style="color: #AFAFAF; width: 793px; text-align: center;">
	You may leave this page at any time and the backup will continue uninterrupted.
</div>
