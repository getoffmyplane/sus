	...
	class pluginbuddy_backupbuddy_admin {
		...
		function admin_menu() {
			if ( isset( $this->_parent->_series ) && ( $this->_parent->_series != '' ) ) {
				...
				add_submenu_page( $this->_parent->_var, $this->_parent->_name.' Backup & Restore', 'Backup & Restore', 'administrator', $this->_parent->_var.'-backup', array(&$this, 'view_backup'));
				add_submenu_page( $this->_parent->_var, $this->_parent->_name.' Malware Scan', 'Malware Scan', 'administrator', $this->_parent->_var.'-malware', array(&$this, 'view_malware'));
				add_submenu_page( $this->_parent->_var, $this->_parent->_name.' Server Info.', 'Server Info.', 'administrator', $this->_parent->_var.'-tools', array(&$this, 'view_tools'));
				add_submenu_page( $this->_parent->_var, $this->_parent->_name.' Scheduling', 'Scheduling', 'administrator', $this->_parent->_var.'-scheduling', array(&$this, 'view_scheduling'));
		...
		function importbuddy_link() {
			...
			echo admin_url( 'admin-ajax.php' ) . '?action=backupbuddy_importbuddy' . $import_pass_query;
		}
